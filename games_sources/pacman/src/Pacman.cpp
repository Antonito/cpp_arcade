...
namespace arcade
{
  ...
  std::vector<std::string> m =
	...
	// Parse map and set powerups, player and enemies position and
	// direction
	...
	*m_map = *other.m_map;
	...
	for (Event const &e : ev)
	  {
		...
		switch (e.kb_key)
		  ...
		  case KB_ARROW_UP:
		    ...
		    if (m_map->at(0, m_player.next().x, m_player.next().y)
		            .getType() == TileType::BLOCK)
	...
	s.push_back(std::make_unique<Sprite>("assets/pacman/", "ghost", 1,
	                                     ".png", "@"));
	s.push_back(std::make_unique<Sprite>("assets/pacman/", "player", 1,
	                                     ".png", "x"));
	s.push_back(std::make_unique<Sprite>("assets/pacman/", "super", 1,
	                                     ".png", "#"));
	s.push_back(std::make_unique<Sprite>("assets/pacman/", "power", 1,
	                                     ".png", "o"));
	return (s);
      }
