      ...
      m_updatePos(0)
{
  ...
  size_t size =
      std::max(m_map->getHeight() / 10, static_cast<size_t>(10));
  ...
  for (size_t player = 0; player < 2; ++player)
  {
    ...
    for (size_t i = 0; i < size; ++i)
    {
	  ...
	  Position(player * (m_map->getWidth() - 3) + 1, y + i));
    }
  }
  m_id = 0;
  m_ball.setBallPos(
      Position(m_map->getWidth() / 2, m_map->getHeight() / 2));
}
...
void Pong::process()
{
  ...
  case PongState::AUTHENTICATING:
    ...
    {
      Network::NetworkPacketData<0, uint8_t> *_pck =
	  reinterpret_cast<Network::NetworkPacketData<0, uint8_t> *>(
	      pck.data);
      rec = true;
      if (ntohl(pck.header.magicNumber) ==
	      NetworkPacketHeader::packetMagicNumber &&
	  _pck &&
	  static_cast<NetworkAction>(ntohl(static_cast<uint32_t>(
	      _pck->action))) == NetworkAction::HELLO_EVENT &&
	  _pck->auth == true)
      {
#if defined(DEBUG)
  ...
  case PongState::SETTING:
    for (NetworkPacket const &pck : m_received)
    {
	  ...
	  reinterpret_cast<
	      Network::NetworkPacketData<0, PongPacket> *>(pck.data);

      if (ntohl(pck.header.magicNumber) ==
	      NetworkPacketHeader::packetMagicNumber &&
	  _pck &&
	  static_cast<NetworkAction>(ntohl(static_cast<uint32_t>(
	      _pck->action))) == NetworkAction::ENTITY_EVENT)
      {
  ...
  case PongState::WAITING:
    ...
    for (NetworkPacket const &pck : m_received)
    {
	  ...
	  _pck &&
	  static_cast<NetworkAction>(ntohl(static_cast<uint32_t>(
	      _pck->action))) == NetworkAction::GAME_EVENT &&
	  _pck->auth == true)
      {
#if defined(DEBUG)
  ...
  case PongState::PLAYING:
    ...
    for (NetworkPacket const &pck : m_received)
    {
	  ...
	  static_cast<NetworkAction>(ntohl(static_cast<uint32_t>(
	      _pck->action))) == NetworkAction::ENTITY_EVENT)
      {
	for (uint32_t i = 0; i < ntohl(_pck->entity.data.update);
	     ++i)
	{
	  if (ntohl(_pck->entity.data.pos.y) >
		  m_player[otherId][0].y &&
	  ...
	  else if (ntohl(_pck->entity.data.pos.y) <
		       m_player[otherId][0].y &&
	...
	if (m_id != 0 && _pck->entity.data.updateBall == true)
	{
	  ...
	  m_ball.setX(static_cast<double>(
	      ntohl(_pck->entity.data.ball.x)));
	  m_ball.setY(static_cast<double>(
	      ntohl(_pck->entity.data.ball.y)));
	}
      }
    }
    break;
...
#endif
      std::unique_ptr<NetworkPacket> createdPck =
	  m_fact.create<1, bool>(
	      NetworkGames::PONG,
	      [&](Network::NetworkPacketData<1, bool> &p) {
		p.action = static_cast<NetworkAction>(
		    htonl(static_cast<uint32_t>(
			NetworkAction::HELLO_EVENT)));
		p.auth = false;
	      });
...
// Waiting for other players
...
#endif
    {
	  ...
	  m_fact.create<1, bool>(
	      NetworkGames::PONG,
	      [&](Network::NetworkPacketData<1, bool> &p) {
		p.action = static_cast<NetworkAction>(htonl(
		    static_cast<uint32_t>(NetworkAction::GAME_EVENT)));
		p.auth = true;
	      });
      NetworkPacket *raw = createdPck.get();
      NetworkPacket pck = *createdPck;
  ...
  case PongState::PLAYING:
  {
    ...
    m_ball.updatePosition(m_player[m_ball.getBallDir()],
			  m_map->getHeight(),
    ...
    if (m_ball[0].x < 0)
    {
      ...
      updateBall = true;
	     ...
	     static_cast<signed>(m_map->getWidth() - 1))
    {
      m_ball.reset(
	  Position(m_map->getWidth() / 2, m_map->getHeight() / 2));
      ballCount = 0;
    ...
    {
      m_player[i].display(*m_map,
			  (m_curTick - m_smoothTick[i]) / 60.0);
    }
    ...
    {
      std::unique_ptr<NetworkPacket> createdPck =
	  m_fact.create<1, PongPacket>(
	      NetworkGames::PONG,
	      [&](Network::NetworkPacketData<1, PongPacket> &p) {
		p.action = static_cast<NetworkAction>(
		    htonl(static_cast<uint32_t>(
			NetworkAction::ENTITY_EVENT)));
		...
		if (m_id == 0)
		{
		  p.entity.data.ball.x =
		      htonl(static_cast<float>(m_ball.getX()));
		  p.entity.data.ball.y =
		      htonl(static_cast<float>(m_ball.getY()));
		}
		else
		{
