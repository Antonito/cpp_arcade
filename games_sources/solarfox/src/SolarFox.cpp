...
SolarFox::getSoundsToLoad() const
{
...
void SolarFox::moveEvilShoot(size_t speed)
{
  ...
  {
    for (std::vector<EvilShoot>::iterator it = m_evilShoot.begin();
	 it != m_evilShoot.end();)
    {
...
void SolarFox::moveEvilDude(size_t speed)
{
  if (m_curTick - m_lastEvilTick > speed)
  {
    ...
    {
      if (e.getShootDir() == Direction::UP ||
	  e.getShootDir() == Direction::DOWN)
...
void SolarFox::enemyShoot(size_t speed)
{
  if (m_curTick - m_lastEvilSalveTick > speed)
  {
    for (Enemy &e : m_enemies)
    {
      ...
      new_shoot.setCurTile(
	  m_map->at(0, new_shoot[0].x, new_shoot[0].y).getType());
