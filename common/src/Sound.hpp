...
namespace arcade
{
  ...
  ///
  /// \struct SoundMode
  /// \brief Contain informations on the sound to play and the way of playing
  /// it
  ///
  struct Sound
  {
    ///
    /// \fn Sound(unsigned int id, SoundMode mode = UNIQUE, SoundType type =
    /// SOUND, float volume = 50.0f);
